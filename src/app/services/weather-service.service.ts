...
export class WeatherServiceService {
  ...
  getCities(city:any):any{
    return this.http.get(`/user/weather/find_city/${city}`)
    .pipe(catchError(this.HttpErrorMsg.handleError));
  }

  dailyForecast(cityid:any):any{
    return this.http.get(`/user/weather/city/daily_report/${cityid}`)
    .pipe(catchError(this.HttpErrorMsg.handleError));
  }

  fivedayForecast(cityid:any):any{
    return this.http.get(`/user/weather/city/5days_report/${cityid}`)
    .pipe(catchError(this.HttpErrorMsg.handleError));
  }

  twelveHourForecast(cityid:any):any{
    return this.http.get(`/user/weather/city/12hour_report/${cityid}`)
    .pipe(catchError(this.HttpErrorMsg.handleError));
  }

  currentCondition(cityid:any):any{
    return this.http.get(`/user/weather/city/currentcondition/${cityid}`)
    .pipe(catchError(this.HttpErrorMsg.handleError));
  }
}